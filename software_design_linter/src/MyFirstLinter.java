...
public class MyFirstLinter {
	...
	static String[] fieldForAnalysisByThisProgram = new String[1];
	...
	public static void main(String[] args) throws IOException {
		...
		for (String className : args) {
			...
			//printTestMethod(classNode);
			fieldForAnalysisByThisProgram = null;
			System.out.println(fieldForAnalysisByThisProgram);
		}
	}
	...
	private static void printClass(ClassNode classNode) {
		...
		// how do I write a lint check to tell if this class has a bad name?
		// doesnt match filename
		// first char not upper case
		// not a noun
		// invalid characters
	...
	private static void printInstructions(MethodNode methodNode) {
		...
		for (int i = 0; i < instructions.size(); i++) {
			...
			if (insn instanceof MethodInsnNode) {
				...
				FieldInsnNode varInsn = (FieldInsnNode) insn;
				...
				// varInsn.setOpcode(opCode);
				// System.out.println("VAR:");
				// System.out.println(varInsn);
				// System.out.println(opCode);
				if((opCode & Opcodes.ILOAD) !=0 ){
					System.out.println("ILOAD");
					System.out.println(varInsn.var);
					//System.out.println(opCode);
				}
				// else if((opCode & Opcodes.ISTORE) != 0){
				// 	System.out.println("ISTORE");
				// 	//System.out.println(opCode);
				// }else if((opCode & Opcodes.ASTORE) != 0){
				// 	System.out.println("ASTORE");
				// 	//System.out.println(opCode);
				// }
				// else if((opCode & Opcodes.ALOAD) != 0){
	...
	private static void printTestMethod(ClassNode classNode){
		List<MethodNode> methods = (List<MethodNode>) classNode.methods;
		for (MethodNode method : methods) {
			// How do you tell if something has default access? (ie no access modifiers?)
